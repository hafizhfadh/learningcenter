...
if [[ "${SKIP_IMAGE_SYNC:-0}" == "1" ]]; then
  log "Step 2/5: Syncing container images (skipped via SKIP_IMAGE_SYNC=1)"
else
  log "Step 2/5: Syncing container images"
  if ! pull_output=$(compose pull 2>&1); then
    printf '%s\n' "${pull_output}" >&2
    if grep -qi 'ghcr.io' <<<"${pull_output}" && grep -qi 'denied' <<<"${pull_output}"; then
      cat <<'EOF' >&2
[HINT] Authentication to ghcr.io failed. Ensure you have logged in with a token that has the
       read:packages scope before rerunning the deployment:
         echo "$GHCR_TOKEN" | docker login ghcr.io -u <github-username> --password-stdin
       Alternatively, rerun with SKIP_IMAGE_SYNC=1 to reuse locally cached images.
EOF
    fi
    exit 1
  fi
