...
verify_required_images() {
  log "Validating availability of container images"
  ...
  if ((${#missing_images[@]} > 0)); then
    printf '[ERROR] Unable to locate remote manifests for the following images:\n' >&2
    local missing
    for missing in "${missing_images[@]}"; do
      printf '  - %s\n' "${missing}" >&2
    done
    printf 'Build and push the listed images to their registries before rerunning the deployment.\n' >&2
    exit 1
  fi
}
